...
def generate_embeds_per_video_id(text_embeds, vid_embeds_pooled, all_vid_ids, pooling_type):
    ...
    # Now keep this version for length info
    cap_per_vid = [text_embeds_per_video_id[v].shape[0] for v in text_embeds_per_video_id]
    # unique_vids = list(text_embeds_per_video_id.keys())
    ...
    return text_embeds_per_video_id, vid_embeds_pooled_per_video_id, cap_per_vid
...
def compute_metrics(lst):
    ...
    metrics["R20"] = 100 * float(np.sum(lst < 20)) / len(lst)
